...
def train(
    ...
    progress_bars = {}
    show_progress = os.getenv("MOE_PEFT_DISABLE_TQDM") is None
    ...
    def task_in_callback(task: TrainTask):
        ...
        if show_progress:
            total_steps = (
                math.ceil(len(task.train_token_data_) / config.micro_batch_size)
                * config.num_epochs
            )
            progress_bars[adapter_name] = tqdm(
                total=total_steps,
                desc=f"{adapter_name}",
                dynamic_ncols=True,
                leave=True,
    ...
    while not dispatcher.check_task_done():
        ...
        for output in outputs:
            ...
            config.step()
            if output.adapter_name in progress_bars:
                progress_bars[output.adapter_name].set_postfix(
                    loss=f"{output.loss.detach().float().item():.4f}"
                )
                progress_bars[output.adapter_name].update(1)
