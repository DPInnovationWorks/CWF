"""
Skeptic Agent：在 Markov 辩论中质疑前序观点并给出判断。
"""
...
class SkepticAgent:
    """质疑者 Agent：优先寻找前序观点中的漏洞与证据不一致点。"""

    SKEPTIC_PROMPT = """
        你是三方辩论中的 *Skeptic（质疑者）*。

        你会收到前序智能体的观点。请结合 claim[text]，必要时自行调用检索工具获取证据，
        严格审查前序观点中的逻辑漏洞、证据不足、夸大或误导之处，并给出你自己的判断。

        要求：
        1. 不要复述前序观点，要明确指出你质疑的点及理由。
        2. 判断必须由证据支撑。
        3. 输出简洁、明确。
    ...
    def respond(
        ...
        *,
        ...
        """
        输入 claim/前序观点，输出 Skeptic 的结构化判断。
        """
        ...
        attach_evidence_fields(
            parsed, messages, debate_round=debate_round, agent_slug="skeptic"
        )
        return parsed
