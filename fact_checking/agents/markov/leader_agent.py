"""
Leader Agent：在 Markov 辩论中综合 Trust / Skeptic 观点并给出裁决。
"""
...
class LeaderAgent:
    """裁决者 Agent：综合前序两方观点，输出事实判断与错误等级。"""

    LEADER_PROMPT = """
        你是三方辩论中的 *Leader（裁决者）*。另外两个角色是 Trust（倾向相信）和 Skeptic（倾向质疑）。
        
        你会收到两个前序智能体的观点。请结合 claim[text]，必要时自行调用检索工具获取证据，综合评估双方观点的优劣，给出你自己的最终裁决。
        
        要求：
        1. 不要复述前序观点，要形成你自己的结论。
        2. 结论必须由证据支撑。
        3. 输出简洁、明确。
        ...
        - 系统会将每条引用映射为全局唯一 id（含辩论轮次与当前角色，如 r1-leader-1-1）；你只需按上规则写批次-条号。
        - 若本轮完全未调用检索，输出 <cited>无</cited>。

        stance 取值限制：{1：强支持, 0.2：中立（弱支持）, 0：中立, -0.2：中立（弱反对）, -1：强反对}

        你必须仅按以下标签格式输出，不要输出标签以外的任何内容：
        <opinion>你的综合裁决意见</opinion>
        <stance>你对 claim 的立场：仅可为 1 / 0.2 / 0 / -0.2 / -1</stance>
    ...
    def __init__(
        ...
        self._agent = create_langchain_agent(
            provider,
            tools=[],
            system_prompt=self.LEADER_PROMPT,
            model=model,
    ...
    def respond(
        ...
        *,
        ...
        """
        输入 claim/前序观点，输出 Leader 的结构化裁决。
        """
        slim_ops = [slim_opinion_for_prompt(o) for o in (previous_opinions or [])]
        opinions_text = json.dumps(slim_ops, ensure_ascii=False, indent=2)

        content = (
            f"[text]\n{claim}\n\n"
            f"[previous opinions]\n{opinions_text}\n\n"
            "你可以自行检索相关信息。请按约定的标签格式返回最终裁决。"
        )
        ...
        attach_evidence_fields(
            parsed, messages, debate_round=debate_round, agent_slug="leader"
        )
        return parsed
